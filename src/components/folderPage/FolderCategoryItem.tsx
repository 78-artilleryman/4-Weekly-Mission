...
const FolderCategoryItem = ({ id, children }: FolderCategoryItemProps) => {
  ...
  return (
    <>
      {folderFilter.name === children ? (
        <SelectedFolder onClick={onClickHandle}>{children}</SelectedFolder>
      ) : (
        <DefaultFolder onClick={onClickHandle}>{children}</DefaultFolder>
      )}
    </>
  );
};
